...
For a given scalar notion of time τ , Time2Vec of τ , denoted as t2v(τ), is a vector of size k + 1 where t2v(τ)[i] is the i th element of t2v(τ), F is a periodic activation function (which is a sine function), and ωi and ϕi are learnable parameters. k is the number of sinusoids (modelling the periodicity in your dataset). i is the index of the k + 1 dimensional vector. It varies between 1 and k, 0th being the linear term. The entire vector is k (from sinusoids) + 1 (from linear term) dimensional vector. The sine function in Time2Vec can be used to capture periodic patterns while the linear term for non-periodic pattern. For more info regarding the paper of Time2Vec, here is the link: https://arxiv.org/pdf/1907.05321.pdf
